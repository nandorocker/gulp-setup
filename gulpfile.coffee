...
#
gulp.task 'html', ->
  # sets base directory according to state; default local
  config.htmlReplaceSrc = 'http://localhost:9000/'
  config.htmlReplaceTpl = '<base href="%s">'

  if config.type
    # Add your final URL here
    config.htmlReplaceSrc = 'http://nandorocker.github.io/nan.do/'
  ...
  gulp.src(config.sourceDir + '/pages/**/*.pug')
...
gulp.task 'cname', ->
  gulp.src('CNAME')
    .pipe(gulpIf(config.type, gulp.dest(config.outputDir)))
